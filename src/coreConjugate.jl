...
function restricted_Gibbs!(zsa, zsb, tia, tib, tja, tjb, cia, cib, cja, cjb, ni, nj, i, j, S, ns, x, b, H, active)
    ...
    if !active
        tia, tja = deepcopy(tia), deepcopy(tja)
    end
    log_p = 0.0
    for ks = 1:ns
        k = S[ks]
        if k != i && k != j
            ...
            Li = log_marginal(x[k], tia, H) - log_marginal(tia, H)
            Lj = log_marginal(x[k], tja, H) - log_marginal(tja, H)
...
function split_merge!(x, z, zs, S, theta, list, N, t, H, a, b, log_v, n_split, n_merge)
    ...
    if ci0 == cj0  # propose a split
        ...
        log_prior_a = log_v[t] + lgamma_(ns + b) - lgamma_(a)
        ...
        # compute acceptance probability
        log_prior_b = log_v[t-1] + lgamma_(ns + b) - lgamma_(a)
...
function sampler(options, n_total, n_keep)
    ...
    for iteration = 1:n_total
        ...
        for i = 1:n
            ...
            for j = 1:t
                cc = list[j]
                log_p[j] = log_Nb[N[cc]] + log_marginal(x[i], theta[cc], H) - log_marginal(theta[cc], H)
            end
            log_p[t+1] = log_v[t+1] - log_v[t] + log(a) + log_marginal(x[i], theta[c_prop], H)
